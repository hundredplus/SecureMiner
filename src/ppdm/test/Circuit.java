 ...
 * <li>The wires are divided into 2 groups: circuit input wires and non-input wires.</li>
 * <li>All the circuit input wires will come before any of the non-input wires.</li>
 ...
 * <li>A player with inputs also have a mapping from the bits those wires represent to variables</li>
 * <li>A player with outputs also have a mapping from the bits those wires represent to variables</li>
 * </ul>
 * <b>Gates:</b>
 * <ul>
 * <li>All gates have a sequential index starting from 0.</li>
 * <li>Each gate will have 2 wires with different index as input.</li>
 * <li>Both input wires of a gate are either circuit input wires or an output wire of a gate with lower index.</li>
 * <li>Each gate will have one wire as output.</li>
 * <li>Each gate has an int number assign to it, which in its binary representation is the gate's truth table.
 * <li>The gate's truth table can not be one of the following: 0, 3, 5, 10, 12 and 15.
 * </ul>
 *  
 * @version 2.0
...
public interface Circuit {
	 ...
	 * All the rest of the player's input wires will be in the following indexs. 
	...
	/**
	 * Gets the number of the players' input wires, which is the players input size in bits.
	 * @param name The name of the player.
	 ...
	 * The entries will be arranged from the msb to the lsb of the player input bits. 
	 * The first entry (position 0) msb will be the players' input msb.
	 * The last entry (position length-1) lsb will be the players' input lsb.   
	 * @param name The name of the player.
	 ...
	 * Each entry contains the variable name (string) as the key, and the indexes of the matching wires (int[]) as the value.
	 * @param name The name of the player.
	 * @return A Map between the player output variable name (String) and it's wires indexs (int[]).
	 */
	public Map getPlayerOutputs(String name);
	 ...
	 * The truth table for a two inputs wires gate range between 0 and 15 (inclusive). 
	 * This number in binary representation (four bits) is the gate's truth table.
	 * For example: The number 14 is 1110 in its binary form and therefor will represent a NAND gate.
	 * The following gates are not supported: 0, 3, 5, 10, 12,15 (the 0/1 consts, the input wires and there negation)<p> 
	 * @param g The gate index.
